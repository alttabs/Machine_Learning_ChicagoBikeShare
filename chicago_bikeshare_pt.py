...
def count_gender(data_list):
    ...
    for index in range(len(gender_list)):
        if gender_list[index] == "Male":
            male += 1
        elif gender_list[index] == "Female":
            female += 1
    return [male, female]
...
def most_popular_gender(data_list):
    ...
    if gender_qtt[0] > gender_qtt[1]:
        answer = "Male"
    elif gender_qtt[1] > gender_qtt[0]:
        gender_qtt = "Female"
    elif gender_qtt[0] == gender_qtt[1]:
        gender_qtt = "Equal"
    return answer
...
def count_user_type(data_list):
    ...
    for i in range(len(user_type_list)):
        if user_type_list[i] == "Subscriber":
            subscriber += 1
        elif user_type_list[i] == "Customer":
            customer += 1
...
answer = "Porque ha entradas que não ha informação de genero alguma, como no Customer"
print("Answer:", answer)
    ...
    for i in range(len(item_types)):
        for j in range(len(column_list)):
            if item_types[i] == column_list[j]:
                count_types += 1
        count_items.append(count_types)
        count_types = 0
